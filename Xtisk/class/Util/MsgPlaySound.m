...
static MsgPlaySound *instanceMsgPlaySound = nil;

@implementation MsgPlaySound


+(MsgPlaySound *)sharedInstance{
    if (!instanceMsgPlaySound) {
        @synchronized([MsgPlaySound class]){
            if (!instanceMsgPlaySound) {
                instanceMsgPlaySound = [[MsgPlaySound alloc] initSystemShake];
            }
        }
    }
    return instanceMsgPlaySound;
}
...
-(void)playReceiveMsg{
    // ReceivedMessage.caf
    if (msgSound == NULL) {
        msgSound = kSystemSoundID_Vibrate;
        NSString *path = [NSString stringWithFormat:@"/System/Library/Audio/UISounds/ReceivedMessage.caf"];
        //[[NSBundle bundleWithIdentifier:@"com.apple.UIKit" ]pathForResource:soundName ofType:soundType];//得到苹果框架资源UIKit.framework ，从中取出所要播放的系统声音的路径
        //[[NSBundle mainBundle] URLForResource: @"tap" withExtension: @"aif"];  获取自定义的声音
        if (path) {
            OSStatus error = AudioServicesCreateSystemSoundID((__bridge CFURLRef)[NSURL fileURLWithPath:path],&msgSound);
    ...
    if (msgSound) {
        AudioServicesPlaySystemSound(msgSound);
    }else{
