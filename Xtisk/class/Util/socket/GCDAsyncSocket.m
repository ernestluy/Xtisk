...
- (void)ensureCapacityForWrite:(size_t)numBytes
{
	...
	if (numBytes > availableSpace)
	{
		...
		uint8_t *newPreBuffer = (uint8_t *)realloc((void *)preBuffer, newPreBufferSize);
...
- (void)doWriteData
{
	...
	if (flags & kSocketSecure)
	{
		if ([self usingCFStreamForTLS])
		{
			...
			const uint8_t *buffer = (uint8_t *)[currentWrite->buffer bytes] + currentWrite->bytesDone;
		...
		else
		{
			...
			{
				const uint8_t *buffer = (uint8_t *)[currentWrite->buffer bytes]
	...
	else
	{
		...
		int socketFD = (socket4FD == SOCKET_NULL) ? socket6FD : socket4FD;
		NSData *dd  = currentWrite->buffer;
      //  NSLog(@"%@",[[dd class] description]);
		const uint8_t *buffer = (uint8_t *)[currentWrite->buffer bytes] + currentWrite->bytesDone;
